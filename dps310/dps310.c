...
static uint8_t g_pressure_prc = DPS310_CFG_PRC_8_TIMES;
static uint8_t g_temperature_prc = DPS310_CFG_PRC_8_TIMES;
...
int16_t read_coefs() {
    ...
    ret = dps310_i2c_read(I2C_DPS310_ADDRESS, DPS310_COEF_REG, &buff[0], 18);
    if (ret != DPS310_OK) return ret;
...
int16_t dps310_configure_temperature(uint8_t data) {
    ...
    g_temperature_prc = DPS310_TMP_CFG_TMP_PRC_MASK & data;
...
int16_t dps310_configure_pressure(uint8_t data) {
    g_pressure_prc = DPS310_PRS_CFG_PM_PRC_MASK & data;
...
static int16_t read_temperature(float *p_temperature) {
    ...
    uint32_t factor;
    ret = get_scale_factor_for(g_temperature_prc, &factor);
    if (ret != DPS310_OK) return ret;
...
int16_t read_pressure(float *p_pressure) {
    ...
    uint32_t factor;
    ret = get_scale_factor_for(g_pressure_prc, &factor);
    if (ret != DPS310_OK) return ret;
...
int16_t get_scale_factor_for(uint8_t prc, uint32_t *p_factor) {
    uint32_t ret = DPS310_OK;

    switch (prc) {
        case  DPS310_CFG_PRC_SINGLE:
            *p_factor = 524288;
            break;
        case  DPS310_CFG_PRC_2_TIMES:
            *p_factor = 1572864;
            break;
        case  DPS310_CFG_PRC_4_TIMES:
            *p_factor = 3670016;
            break;
        case  DPS310_CFG_PRC_8_TIMES:
            *p_factor = 7864320;
            break;
        case  DPS310_CFG_PRC_16_TIMES:
            *p_factor = 253952;
            break;
        case  DPS310_CFG_PRC_32_TIMES:
            *p_factor = 516096;
            break;
        case  DPS310_CFG_PRC_64_TIMES:
            *p_factor = 1040384;
            break;
        case  DPS310_CFG_PRC_128_TIMES:
            *p_factor = 2088960;
            break;
        default:
